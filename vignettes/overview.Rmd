...
output <- input %>%
  mutate(access = usage_to_access(usage, usage_rate),
         npc = access_to_npc(access),
         anpcd = npc_to_anpcd(npc, distribution_freq = distribution_freq, net_loss_function = net_loss_map, half_life = half_life))
...
The relationship between net access and nets per capita is derived from monthly data for the latest available year (2020) across African countries, and plotted below (corresponding to Figure 4 in paper). There are three options in the `access_to_npc()` function
...
access <- seq(0, 1, 0.01)
npc_loess <- access_to_npc(access, type = "loess")
npc_loess_extrapolate <- access_to_npc(access, type = "loess_extrapolate")
npc_linear <- access_to_npc(access, type = "linear")
...
Under default assumptions, nets are assumed to be lost according to a smooth-compact function after distribution, which depends on the estimated net half life. The proportion of nets retained over a 3-year distribution cycle with a median retention time / half-life of 1.64 years is shown below. 
